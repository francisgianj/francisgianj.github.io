...
>(({ children, ...props }, ref) => {
  ...
  return (
    <a
      ...
      ref={ref}
      className={cn(
        location === props.href ? "dark:bg-slate-200/20 bg-slate-800/20" : "",
        "font-bold text-slate-800 dark:text-slate-200 block text-lg sm:text-base sm:inline-block p-1 sm:px-3 sm:py-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 transition-all"
      )}
    >
      {children}
    </a>
